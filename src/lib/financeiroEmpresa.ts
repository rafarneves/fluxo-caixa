...
import { formatarDataServidor, getContextoConfiguracoes } from './configuracoes-server';
...
export async function calcularFinanceiroEmpresa() {
    ...
    const faturamentoMensal = contratosData.reduce((total: number, contrato: any) => total + Number(contrato.valor), 0);

    const despesasTotal = despesasData.reduce((total: number, despesa: any) => total + Number(despesa.valor), 0);

    const custosTotal = custosData.reduce((total: number, custo: any) => total + Number(custo.valor), 0);
