...
export default async function ContratoDetalhe({
  ...
  if (!contrato) {
    return <div className="text-red-400 text-2xl">Contrato não encontrado.</div>;
  }
    ...
    <main className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-400 mt-2">Detalhes do contrato</p>
        </div>
      ...
      <div className="rounded-3xl border border-zinc-800 bg-[#161B22] p-8">
        <h2 className="text-2xl font-bold mb-6">Recebimentos</h2>

        <div className="space-y-3">
          {recebimentosData.map((recebimento) => (
            <div
              key={recebimento.id}
              className="rounded-xl bg-zinc-900/60 p-5 flex justify-between"
            >
              <div>
