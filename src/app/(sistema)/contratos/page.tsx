...
export default async function ContratosPage() {
  ...
  const cancelados = contratosData.filter(
    (contrato: any) => contrato.status === "Cancelado"
  ).length;
    ...
    <div className="space-y-10">
      <div className="flex items-start justify-between">
        ...
        <Link
          ...
          className="rounded-2xl bg-green-500 px-6 py-4 font-bold text-black transition-all duration-300 hover:bg-green-400 hover:-translate-y-1 hover:shadow-xl hover:shadow-green-500/20"
        >
          + Novo Contrato
        </Link>
      </div>
          ...
          <tbody>
            {contratosData.map((contrato: any) => (
              <tr
                ...
                className="border-t border-zinc-800 hover:bg-black/20 transition"
              >
                <td className="p-5">
                  <Link
                ...
                <td className="p-5 text-right">
                  {contrato.status !== "Cancelado" && (
                    ...
                    >
                      <button className="bg-red-500 hover:bg-red-400 text-white px-5 py-2 rounded-xl font-semibold transition hover:-translate-y-0.5">
                        Cancelar
                      </button>
                    </form>
                  )}
