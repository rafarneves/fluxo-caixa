...
export default function DREMetricCard({
  ...
  return (
    <div className="bg-[#161B22] border border-zinc-800 rounded-2xl p-6 hover:border-green-500/30 transition-all duration-300">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-zinc-400 text-sm">{titulo}</p>

          <h2 className={`text-3xl font-bold mt-3 ${cores[cor].texto}`}>{valor}</h2>

          {subtitulo && <p className="text-zinc-500 text-sm mt-2">{subtitulo}</p>}
        </div>
