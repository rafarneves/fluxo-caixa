...
function formatCompact(value?: number) {
  ...
  if (abs >= 1000000) {
    return `${numero < 0 ? "-" : ""}R$ ${(abs / 1000000).toFixed(1)}M`;
  }

  if (abs >= 1000) {
    return `${numero < 0 ? "-" : ""}R$ ${(abs / 1000).toFixed(1)}k`;
  }
    ...
    <section className="rounded-3xl border border-zinc-800 bg-gradient-to-b from-[#171F2B] to-[#111827] p-8">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">Saúde Financeira</p>
        </div>
      </div>
